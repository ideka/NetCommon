...
public class Comparer2<T>(NullableComparison<T> comparison) : Comparer<T>
{
    private readonly NullableComparison<T> _compareFunction = comparison ?? throw new ArgumentNullException(nameof(comparison));

    public override int Compare(T? x, T? y) => _compareFunction(x, y);
}

public sealed class DictionaryEnumerator<TKey, TValue>(IDictionary<TKey, TValue> value) : IDictionaryEnumerator, IDisposable where TKey : notnull
{
    private readonly IEnumerator<KeyValuePair<TKey, TValue>> _impl = value.GetEnumerator();
